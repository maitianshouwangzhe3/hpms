...
local function mysql_eventloop()
    ...
    while true do
        ...
        if not res then
            ...
            hpms.co_resume(co, res, badresult)
        else
            if err ~= "again" then
                local co = tab_remove(backlog, 1)
                hpms.co_resume(co, res, err)
            else
                local mres = {res}
...
local function proxy_metafunc(tab, cmd)
    local function wait_for_response(self, ...)
        ...
        if res then
            backlog[#backlog+1] = hpms.co_running()
            local fd = self[1].sock
            hpms.co_attach(fd)
            res, err = hpms.co_yield()
            hpms.co_detach(fd)
        end
        return res, err
    end
