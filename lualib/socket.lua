local ae = require "hpms.ae"
local anet = require "hpms.anet"
local buffer = require "hpms.buffer"
...
function _M.shutdown(fd, how)
    local s = socket_pool[fd]
    if s then
        anet.shutdown(fd, how or 0)
    end
end 
...
local function ev_client_handler(s, readable, writable, _)
    assert(s)
    local runfd = assert(hpms.co_runfd(s.co))
    if readable then
        ...
        if not n then
            ...
            if s.fd == runfd then
                return hpms.co_resume(s.co, n, err)
            else
                s.errmsg = err
                return
            end
    ...
    if writable then
        ...
        if s.fd == runfd and s.writable and ok then
            s.writable = false
            ae.enable(aefd, s.fd, true, false)
            hpms.co_resume(s.co, true)
        end
    end
end
...
local function ev_connect_handler(s, readable, writable, errevent)
    if errevent then
        hpms.co_resume(s.co, nil, errevent)
        return
    end
...
function _M.new_poll()
    aefd = ae.create()
    ae.register({
        update_time = hpms.update_cache_time,
...
function _M.listen(endpoint, on_accept)
    ...
    local co = hpms.co_create(function ()
        while true do
...
function _M.read(fd, sz)
    ...
    s.read_need = sz
    hpms.co_attach(fd)
    local ok, err = hpms.co_yield()
    hpms.co_detach(fd)
    s.read_need = false
    return ok, err
end

function _M.readall(fd)
    local s = assert(socket_pool[fd])
    if s.errmsg then
        return nil, s.errmsg
    end
    local buf = s.rbuffer:readn(0)
    if buf then
        return buf
    end
    s.read_need = 0
...
function _M.connect(ip, port, opts)
    ...
    if spool.backlog >= 0 then
        ...
        if spool.connections > spool.pool_size then
            ...
            spool.wait_timer[running] = hpms.add_timer(spool.wait_timeout, function ()
                local runco = tab_remove(spool.wait, 1)
                hpms.co_resume(runco, nil, "timeout")
            end)
            hpms.co_attach(-1)
            local fd, err = hpms.co_yield()
            hpms.co_detach(-1)
            if err then
                spool.connections = spool.connections - 1
            else
