...
lwait(lua_State *L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "hpms.update_time");
    lua_call(L, 0, 0);
    if (retmask & mask) {
...
lregister(lua_State *L) {
    ...
    lua_setfield(L, LUA_REGISTRYINDEX, "hpms.update_time");
    lua_getfield(L, 1, "ev_handler");
    luaL_checktype(L, -1, LUA_TFUNCTION);
    lua_setfield(L, LUA_REGISTRYINDEX, "hpms.ev_handler");
    return 0;
}
...
lpoll(lua_State *L) {
    ...
    lua_pushvalue(L, 4);
    lua_call(L, 0, 0);
    lua_getfield(L, LUA_REGISTRYINDEX, "hpms.ev_handler");
    for(int i = 0; i < n; i++)
    {
        lua_pushvalue(L, 5);
